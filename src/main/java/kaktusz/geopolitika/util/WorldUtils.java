...
public class WorldUtils {
	@Nullable
	public static RayTraceResult rayTraceBlocksDontLoad(World world, Vec3d vec31, Vec3d vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock)
	{
		if (!Double.isNaN(vec31.x) && !Double.isNaN(vec31.y) && !Double.isNaN(vec31.z))
		{
			if (!Double.isNaN(vec32.x) && !Double.isNaN(vec32.y) && !Double.isNaN(vec32.z))
			{
				...
				IBlockState iblockstate = getBlockStateDontLoad(world, blockpos);
				...
				while (k1-- >= 0)
				{
					...
					IBlockState iblockstate1 = getBlockStateDontLoad(world, blockpos);
